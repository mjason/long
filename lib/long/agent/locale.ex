defmodule Long.Agent.Locale do
  ...
  fallback chain:
      ...
      channel (credential) → member → group → platform-detected → system default

  Owner-set values (credential / member / group, configured in `/manage`)
  ...
  @doc """
  Locale for a group member directly (member → group → default).
  ...
  defp member_locale(member_id) when is_binary(member_id) do
    case Agent.get_member(member_id, load: [:group]) do
  ...
  defp member_chain(member) do
    blank(Map.get(member, :locale)) || group_locale(Map.get(member, :group))
  end

  defp group_locale(%{locale: locale}), do: blank(locale)
  defp group_locale(_), do: nil
